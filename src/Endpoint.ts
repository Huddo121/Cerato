...
export type ResponseCode = 200 | 201 | 202 | 301 | 302 | 307 | 308 | 400 | 401 | 403 | 404 | 429 | 500 | NonContentfulResponseCode;

export type OutputMapping = { [c in ResponseCode]?: z.ZodType };

export type Accepts = "json" | "multipart-form";

export type EndpointParams<M extends Methods, I, O extends OutputMapping | undefined, H, A extends Accepts> = {
  method: M;
...
type AddOutput<O extends OutputMapping | undefined, C extends ResponseCode, Z extends z.ZodType> = O extends undefined ? { [c in C]: Z } : O & { [c in C]: Z };
...
type MappedApi<A extends API, R> = { [K in keyof A as string]: A[K] extends AnyEndpoint | AnyMulti ? R : MappedApi<A[K] & API, R> };

const mapApiRecurse = <A extends API, R>(api: A, f: (path: PathParts, endpoint: AnyEndpoint | AnyMulti) => R, path: PathParts): MappedApi<A, R> => {
...
export const mapApi = <A extends API, R>(api: A, f: (path: PathParts, endpoint: AnyEndpoint | AnyMulti) => R): MappedApi<A, R> => {
 ...
 */
export type Possibly<K extends string, E extends AnyEndpoint | API | undefined> = E extends AnyEndpoint | API ? { [k in K]: E } : {};
...
> = Possibly<"GET", GET> & Possibly<"POST", POST> & Possibly<"PUT", PUT> & Possibly<"PATCH", PATCH> & Possibly<"DELETE", DELETE> & Possibly<"children", C>;
...
export class Multi<M extends AnyEndpointMapping> {
  ...
  get children(): ChildrenForEndpointMapping<M> {
    ...
    return (this.endpointMapping as { children?: unknown }).children as ChildrenForEndpointMapping<M>;
  }
}
...
> {
  ...
  output<C extends ResponseCode, Z extends z.ZodType = ZodUndefined>(code: C, validator?: Z): Endpoint<M, I, AddOutput<O, C, Z>, Q, H> {
    const updatedValidators = Object.assign({}, this.outputValidators ?? {}, { [code]: validator ?? z.undefined() }) as AddOutput<O, C, Z>;
  ...
  static json(): Endpoint<"GET", undefined, undefined, undefined, undefined, "json"> {
  ...
  static multipart(): Endpoint<"GET", undefined, undefined, undefined, undefined, "multipart-form"> {
  ...
  >(overrides: Partial<EndpointParams<M2, I2, O2, H2, A2>>): Endpoint<M2, I2, O2, Q2, H2, A2> {
...
export type ChildrenForEndpointMapping<MP extends AnyEndpointMapping> =
  MP extends EndpointMapping<infer _G, infer _P, infer _PUT, infer _PATCH, infer _D, infer C> ? C : never;
